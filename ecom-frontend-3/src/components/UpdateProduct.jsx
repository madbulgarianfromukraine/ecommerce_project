...
const UpdateProduct = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
       ...
       const imageFile = await converUrlToFile(responseImage.data,response.data.imgName);
        setImage(imageFile);     
  ...
  const handleSubmit = async(e) => {
    ...
    if(updateProduct.available && updateProduct.stock == '0'){
      alert("Stock quantity cannot be zero if product is available");
      return;
    }
    if(!updateProduct.available && updateProduct.stock > 0){    
      alert("Stock quantity must be zero if product is not available");
      return;
    }
